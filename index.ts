...
async function askInput() {
  const answers = await inquirer.prompt([
    /* Pass your questions in here */
    {
      ...
      message: chalk.inverse.bold(
        "WHAT ARITHMATIC OPERATION DO YOU WANT TO PERFORM? \n"
      ),
  ...
  if (answers.operator == "Addition") {
    console.log(
      chalk.green(`${answers.n1} + ${answers.n2} = ${answers.n1 + answers.n2}`)
    );
  } else if (answers.operator == "Subtraction") {
    console.log(
      chalk.yellow(`${answers.n1} - ${answers.n2} = ${answers.n1 - answers.n2}`)
    );
  } else if (answers.operator == "Multiplication") {
    console.log(
      chalk.blue(`${answers.n1} * ${answers.n2} = ${answers.n1 * answers.n2}`)
    );
  } else if (answers.operator == "Division") {
    console.log(
      chalk.magentaBright(
        `${answers.n1} / ${answers.n2} = ${answers.n1 / answers.n2}`
      )
    );
  }
}
